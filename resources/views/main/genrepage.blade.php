...
<div class="col-lg-7">
  <div class="card borderlatest shadow-sm" style="width:43.4rem; height:55.5rem">
   ...
   <div class="card-body">
    ...
    <div class="row">
    @foreach($genre as $genreobj)
    <div class="card pr-2 pb-3">
    <a href="{{ route('showanimasi',$genreobj->id) }}">
    <img src="{{ asset('storage/'.$genreobj->image) }}" width="160rem" height="215rem" class="fadeoutimage shadow-sm">
    </a>
    <span class="textlimited" style="font-size:14.2px" maxlength="20">{{ $genreobj->title }}</span>
    </div>
    @endforeach
  ...
  <div id="accordion">
    ...
    <div class="card mt-3 shadow-sm" style="width:15rem;" >
      ...
      <div>
        ...
        <button onclick="location.href='{{ route('genrepage',$tagobj->id) }}'" type="button" class="btn btn-xs btn-info" style="margin-left:-0.7rem; margin-right:0.7rem; margin-bottom:0.4rem"><b>{{ $tagobj->tagname }}</b></button>
        @endforeach
