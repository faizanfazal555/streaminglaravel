...
<div class="col-lg-7">
  <div class="card borderlatest shadow-sm" style="width:43.4rem; height:55.5rem">
   ...
   <div class="card-body">
    ...
    <a href="{{ route('showanimasi',$animasiobj->id) }}">
    <img src="{{ asset('storage/'.$animasiobj->image) }}" width="160rem" height="215rem" class="fadeoutimage shadow-sm">
    </a>
    <span class="textlimited" style="font-size:14.2px" maxlength="20">{{ $animasiobj->title }}</span>
    </div>
    @endforeach

  </div>
    </div>
    <div class="card-footer justify-content-center" style="background-color:white; border:none; text-align:center">
     {{ $animasi->appends(['searching'=> request()->query('searching')])->links() }}
    </div>
   </div>
  </div>
